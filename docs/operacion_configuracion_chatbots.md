...
- Configuración persistente por bot
  ...
  - Esquema (modelo): `services/chatbots/models.py` → `BotSettings` con:
    ...
    - `features`: `use_rules` (FAQ/reglas), `use_rag`.
...
- Base de conocimiento (RAG)
  - Dataset por defecto: `knowledge/faqs/municipal_faqs.json`.
...
- Desde el Portal (recomendado, sin tocar archivos)
  ...
  3. Ajustá:
     ...
     - Comportamiento: `usar reglas` (FAQ/reglas), `usar RAG`.
...
- Consejos prácticos
  - Municipal (informativo): `temperature` 0.6–0.8, `top_p` 0.8–0.95, `max_tokens` 200–400; `use_rules=true`, `use_rag=true`.
